...
bool InterfaceManager::writeMotorState( dynamixel_control_items_t item_id,
	int motor_number,
	uint32_t write_value ) {
	return writeDynamixelRegister(
		get_id( motor_number ), dynamixel_control_items_[item_id].address,
		dynamixel_control_items_[item_id].data_length, write_value );
}

bool InterfaceManager::writeDynamixelRegister( uint8_t id, uint16_t addr,
	uint8_t length,
	uint32_t value ) {
	...
	if ( length == 1 ) {
		comm_result = packetHandler_->write1ByteTxRx(
			portHandler_, id, addr, (uint8_t)value, &dynamixel_error );
	} else if ( length == 2 ) {
		comm_result = packetHandler_->write2ByteTxRx(
			portHandler_, id, addr, (uint16_t)value, &dynamixel_error );
	} else if ( length == 4 ) {
		comm_result = packetHandler_->write4ByteTxRx(
			portHandler_, id, addr, (uint32_t)value, &dynamixel_error );
	}
...
void InterfaceManager::doSyncWrite( dynamixel_control_items_t item_id,
	std::vector<double>* ref ) {
	bool do_write = false;

	// Initialize GroupSyncWrite instance
	ControlItem item = dynamixel_control_items_[item_id];
	dynamixel::GroupSyncWrite groupSyncWrite( portHandler_, packetHandler_,
		item.address, item.data_length );
	/*
  ...
  convert_torque_value(joint_setpoints_.effort[i], i);
                  ...
                  // Add Dynamixel goal position value to the Syncwrite
  parameter storage
                  uint8_t dxl_addparam_result =
  groupSyncWrite.addParam(dynamixel_[i].id_, param_goal_current);

                  if(!dxl_addparam_result) {
                          ROS_ERROR("ID:%d] groupSyncRead addparam failed",
  dynamixel_[i].id_);
                          //success = false;
                  }
          }
